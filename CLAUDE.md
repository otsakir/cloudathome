...
Docker Compose needs `CAH_HTTP_PORT`/`CAH_HTTPS_PORT`/`CAH_API_PORT`/
...
| **Django** (`tunnelagent`: Django + sshd) | a configurable API port (`CAH_API_PORT`, `.env`; default 8000), a configurable SSH port (`CAH_SSH_PORT`, `.env`; default 8022) | REST API + web UI for managing homes/proxy mappings; SSH server that accepts reverse tunnels from home networks |
...
- **Standard HTTP/HTTPS/API ports are configurable, not hardcoded** (`CAH_HTTP_PORT`/`CAH_HTTPS_PORT`/`CAH_API_PORT`, `.env`; default 80/443/8000), specifically so more than one CloudAtHome instance can run on the same host — each needs its own host IP, or its own non-standard ports, since only one process can ever bind real `0.0.0.0:80`/`:443` at a time. `CAH_HTTP_PORT`/`CAH_HTTPS_PORT` are threaded into `haproxy.cfg`'s `https_frontend`/`http_frontend` `bind "*:$VAR"` lines the same way as the alternate range below, and are also read by Django (`settings.CAH_HTTP_PORT`/`CAH_HTTPS_PORT`) as `tunnels.services.DEFAULT_SCHEME_PORTS` — the default `public_port` a mapping gets when a home omits it — so they must always match what `haproxy.cfg` actually binds for this instance. `CAH_API_PORT` only affects `tunnelagent`'s published host port in `compose.yaml` (the container-internal Django port stays `8000`).
- **Routing Django through HAProxy instead of `CAH_API_PORT`**: `CAH_HOSTNAME` (`.env`, unset by default) is an opt-in alternative to reaching Django only via `CAH_API_PORT` — when set, `manage.py reconcile_admin_route` (run from `docker/django/entrypoint.sh` on every container start, since map files start empty on restart, same as home mappings) seeds a static `host_http_backends.map` entry `<CAH_HOSTNAME>:<CAH_HTTP_PORT> → cah_django_backend` via `HAProxyService.ensure_admin_route`, where `cah_django_backend` is a fixed (not per-home-generated) backend declared directly in `haproxy.cfg` pointing at `tunnelagent:8000`. HTTP only for now — Django has no TLS termination of its own (`manage.py runserver`), so there's no HTTPS/SNI counterpart. `BaseDomainService.validate` refuses to let any home register `CAH_HOSTNAME`, or a domain overlapping it, as a base domain — same overlap check already used between homes' own domains, just against this one reserved value.
...
src/
    ...
    │   │   └── reconcile_admin_route.py   # Seeds the CAH_HOSTNAME map entry on startup
