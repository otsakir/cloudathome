...
Docker Compose needs `HTTP_INBOUND_PORT_RANGE`/`HTTPS_INBOUND_PORT_RANGE`/
...
| **Django** (`tunnelagent`: Django + sshd) | 8000 (API), a configurable SSH port (`CAH_SSH_PORT`, `.env`; default 8022) | REST API + web UI for managing homes/proxy mappings; SSH server that accepts reverse tunnels from home networks |
